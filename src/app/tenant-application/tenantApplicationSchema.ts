...
export const rentalApplicationSchema = z.object({
  ...
  moveInDate: z.string().nonempty("Move-in date is required"), // Could also be z.date()
  ...
  // Previous Addresses (optional array; include one or two as needed)
  previousAddresses: z
    .array(
      z.object({
  ...
  // Previous Employer Details (if changed job in past 2 years)
  previousEmployer: z
    .object({
  ...
  guarantor: z
    .object({
      ...
      // If guarantor has changed address in the past two years
      newAddressDetails: z
        .object({
          address: z.string().optional(),
          postCode: z.string().optional(),
...
// Exporting the inferred TypeScript type can be useful for your React Hook Form
...
export const defaultValues: RentalApplicationForm = {
  ...
  // Previous Addresses (optional, starting as empty)
  previousAddresses: [],

  // Living Situation
  // (Using the first enum value as a default, adjust as needed)
  currentSituation: "homeowner",
  landlordDetails: undefined, // Only set when currentSituation === "rented"
